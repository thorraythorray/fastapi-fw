...
from app.core.config import LOGIN_URL, settings, SKIP_RESPONSE_FORMAT_URLS
from app.core.pagination import FailedResponseModel, SuccessResponseModel
...
class ResponseMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if response.headers.get("Content-Type") == "application/json" and url_pattern.startswith('/api'):
            if (
                (url_pattern == LOGIN_URL and settings.debug)
                or url_pattern in SKIP_RESPONSE_FORMAT_URLS
            ):
                return response
            response_body = b""
